      ...
      <div className='bg-black h-screen flex justify-center items-center'>
          <div className='bg-[#011627] rounded-lg border-[#607B96] justify-between flex flex-col border h-[85%] w-[90%]'>
              <div className='flex justify-between text-[#607B96] h-12 items-center border-b-[1px] border-[#607B96] '>
                ...
                <div className={`flex  h-12 w-24 divide-[#607B96]  items-center cursor-pointer ${activeTab == 1 ? 'border-b-2 border-[#FEA55F]' : ''}`} onClick={() => handleTabClick(1)}>
                    <p className='ml-5'>_hello</p>
                  </div>
                  <div className={`flex  h-12 w-28 border-l border-[#607B96] items-center cursor-pointer ${activeTab == 2 ? 'border-b-2 border-b-[#FEA55F]' : ''}`} onClick={() => handleTabClick(2)}>
                      <p className='ml-5'>_about-me</p>
                    </div>
                  <div className={`flex  h-12 w-24 border-l border-[#607B96]  items-center cursor-pointer ${activeTab == 3 ? 'border-b-2 border-b-[#FEA55F]' : ''}`} onClick={() => handleTabClick(3)}>
                        <p className='ml-5'>_projects</p>
                    </div>
                      <div className='flex border-l border-[#607B96] h-12   items-center'></div>
                </div>
                </div>
                <div className={`border-[#607B96] border-l  h-12 flex items-center cursor-pointer ${activeTab == 4 ? 'border-b-2 border-b-[#FEA55F]' : ''}`} onClick={() => handleTabClick(4)}>
              ...
              {/* <div  className={activeTab === 3 ?'flex  font-Fira_code h-[90%] gap-16  justify-center ' : 'hidden'}>
                  <_home />
              </div>
              <div  className={activeTab === 4 ?'flex  font-Fira_code h-[90%] gap-16  justify-center ' : 'hidden'}>
                  <_home />
              </div> */}
